...
grep "Epoch.*HR" log18_bf12_fpbpmod_correct256to16_v100 |awk '{print substr($2,1,length($2)-1) " " substr($5,1,length($5)-1)}' > p18_bf12_fpbpmod_correct256to16_v100
grep "Epoch.*HR" log19_bf10_fpbpMod_merge_v100 |awk '{print substr($2,1,length($2)-1) " " substr($5,1,length($5)-1)}' > p19_bf10_fpbpMod_merge_v100
