...
class RNNBase(Module):
    ...
    def forward(self, input, hx=None):
        ...
        # SSY haha it is where magic is
        #print("SSU hx type "+str(type(hx)))
        #for l in hx:
        #  print("hx list "+str(type(l)))
        #print("SSU hx "+str(hx))
        wgt = [[ bf16cutfp.apply(w) for w in ww] for ww in self.all_weights]
        ipt = bf16cutfp.apply(input)
        hx1 = tuple(bf16cutfp.apply(hxe) for hxe in hx)
        output, hidden = func(ipt, wgt, hx1, batch_sizes)
        output = bf16cutbp.apply(output)
